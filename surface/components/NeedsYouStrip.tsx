...
/** Priority tint for the card glass pill header */
function getPriorityTint(item: NeedsYouItem): string | undefined {
  if (item.urgent && item.important) return theme.priority.important.tint;
  if (item.important) return theme.priority.important.tint;
  if (item.urgent) return theme.priority.urgent.tint;
  return undefined;
}
...
export function NeedsYouStrip({ items, onNavigate, onExpand, headerColor }: NeedsYouStripProps) {
        ...
        {items.map((item) => (
          <View key={item.session.id} style={styles.cardWrapper}>
            ...
            <ChatCard
              ...
              priorityTint={getPriorityTint(item)}
            />
          </View>
        ))}
      </ScrollView>
